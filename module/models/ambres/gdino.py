...
def _cross_label_nms(
    ...
    """
    서로 다른 레이블 간 겹치는 박스(IoU > iou_thr)를 제거합니다.

    예) box와 cube가 같은 위치에 탐지되면 낮은 score 쪽을 제거.
    → "box를 cube로 중복인식"하는 경우 cube 탐지에서 해당 박스를 제거하여
      진짜 cube만 남깁니다.
    """
    ...
    for label, det in all_dets:
        # 이미 유지된 다른 레이블 박스와 겹치면 제거
        dominated = any(
            kept_label != label and _iou(det["box"], kept_det["box"]) > iou_thr
