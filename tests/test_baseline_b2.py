...
from baselines.b2_no_memory import METHOD_NAME, run_b2_no_memory
...
class TestB2HappyPath:
    def test_clear_checkpoint_returns_continue(self, image_path, mock_handler):
        mock_handler.handle.side_effect = [_reset(), _query_task_ambiguous(False)]
        with patch("baselines.b2_no_memory._make_handler", return_value=mock_handler):
        ...
        assert result.metadata["checkpoint"] == "C1"
        assert result.metadata["uses_checkpoint"] is True
        assert result.metadata["stores_g0"] is False

    def test_ambiguous_checkpoint_returns_ask_with_question(self, image_path, mock_handler):
        ...
        assert result.metadata["checkpoint"] == "C2"
        assert "ambiguous" in result.reason.lower()


class TestB2AmbiguityKeys:
    def test_task_ambiguous_key_is_supported(self, image_path, mock_handler):
        ...
        with patch("baselines.b2_no_memory._make_handler", return_value=mock_handler):
            result = run_b2_no_memory(image_path, TASK, session_id=SESSION)
        assert result.decision == Decision.ASK
...
class TestB2CallSequence:
    def test_calls_reset_then_query_only(self, image_path, mock_handler):
        mock_handler.handle.side_effect = [_reset(), _query_task_ambiguous(False)]
        with patch("baselines.b2_no_memory._make_handler", return_value=mock_handler):
            run_b2_no_memory(image_path, TASK, checkpoint="C1", session_id=SESSION)
    ...
    def test_injected_handler_skips_make_handler(self, image_path, mock_handler):
        mock_handler.handle.side_effect = [_reset(), _query_task_ambiguous(False)]
        with patch("baselines.b2_no_memory._make_handler") as make_handler:
            result = run_b2_no_memory(
                image_path,
                TASK,
    ...
    def test_query_failure_raises_runtime_error(self, image_path, mock_handler):
        ...
        with patch("baselines.b2_no_memory._make_handler", return_value=mock_handler):
            with pytest.raises(RuntimeError, match="AmbRes query failed"):
                run_b2_no_memory(image_path, TASK, session_id=SESSION)
