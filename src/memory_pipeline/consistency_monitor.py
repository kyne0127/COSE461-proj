...
def _cluster_coords(
    ...
    """Molmo 격자 포인트를 물체별 중심점으로 병합.

    Molmo는 단일 물체 표면에 격자 패턴으로 여러 포인트를 반환한다.
    eps 내의 포인트들을 연결 성분(connected component)으로 묶어
    각 클러스터의 중심점 1개로 환원한다.

    Args:
        coords: [[x, y], ...] Molmo 검출 좌표 목록.
        eps:    같은 클러스터로 묶을 최대 픽셀 거리 (기본 40px).
                Molmo 격자 간격(~15px) 보다 크고,
                별개 물체 간 거리(보통 100px+) 보다 작아야 함.
    ...
    for i in range(n):
        ...
        while queue:
            cur = queue.pop()
            for j in range(n):
                if labels[j] == -1 and _euclidean(pts[cur], pts[j]) <= eps:
                    labels[j] = cluster_id
                    queue.append(j)
        cluster_id += 1
    ...
    for cid in range(cluster_id):
        members = [pts[i] for i in range(n) if labels[i] == cid]
        cx = int(sum(p[0] for p in members) / len(members))
        cy = int(sum(p[1] for p in members) / len(members))
...
def get_checkpoint_detections(
    ...
    # Molmo 격자 포인트 → 클러스터 중심점으로 정규화
    # (단일 물체에 12개 등의 포인트를 반환하는 Molmo 특성 보정)
    raw = result["detections"]
    return {
        label: _cluster_coords(coords)
...
def get_checkpoint_detections_from_frame(
    frame: "np.ndarray",
    g0: dict[str, Any],
    handler: Any,
    session_id: str = "checkpoint",
) -> dict[str, list[list[int]]]:
    """Like get_checkpoint_detections but accepts a numpy array directly.

    Used by MemoryFSM which holds video frames as arrays, not file paths.
    """
    import numpy as np
    ...
    if str(_src) not in sys.path:
        sys.path.insert(0, str(_src))
    from extraction.ambres_g0_extractor import _unwrap_handler_result
    ...
    raw = result["detections"]
    return {label: _cluster_coords(coords) for label, coords in raw.items()}
...
def check_grounding_ensemble(
    ...
    # Molmo count=0은 "객체가 실제로 없음"을 강하게 시사한다.
    # GDino는 text query force-matching으로 다른 object를 대상 object로 잘못 탐지할 수 있음.
    # → Molmo=0이면 GDino detection을 force-match로 간주하고 Molmo를 신뢰 (INVALID 유지).
    # → 단, GDino가 g0 좌표 근처에서 탐지했다면 Molmo가 놓친 것이므로 CLEAR.
    if molmo_state in invalid_states and gdino_ck_count >= 1:
        ...
        if _euclidean(g0_coord, nearest) <= threshold:
            # GDino가 예상 위치 근처에서 탐지 → Molmo가 놓친 것 → CLEAR
            return GroundingState.CLEAR, Decision.CONTINUE
        # GDino가 멀리 있는 객체를 탐지했고 Molmo는 0개:
        # Molmo를 신뢰 → INVALID (GDino force-match 가능성 높음)
        # (객체가 정말 이동한 경우 Molmo도 새 위치에서 탐지해야 함)
        return molmo_state, _STATE_TO_DECISION[molmo_state]
